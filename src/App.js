...
// --- SABİT VERİLER (PERFORMANS İÇİN DIŞARI ALINDI) ---
...
const officeDetails = {
  eregli: {
    name: "Ereğli Şubesi",
    city: "Konya",
    address: "Yunuslu mh. uğur mumcu caddesi 35/A Ereğli/Konya",
    phone: "0533 638 7000",
    authNo: "4202207",
  },
  karaman: {
    name: "Karaman Şubesi",
    city: "Karaman",
    address: "İmaret mahallesi 173. sokak No:3/A Karaman",
    phone: "0543 306 14 99",
    authNo: "7000161",
  },
  konya: {
    name: "Konya Şubesi",
...
const locationData = {
  ...
  Mersin: {
    ...
    Mezitli: ["Akdeniz", "Davultepe", "Fatih", "Kuyuluk", "Menderes"],
  },
  Eskişehir: {
...
const options = {
  ...
  flatCount: [
    "Müstakil",
    ...Array.from({ length: 15 }, (_, i) => (i + 2).toString()),
  ],
  age: [
    "Sıfır",
    "İnşaat Hali",
...
export default function RealEstateAssistant() {
  ...
  const handleLogoChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onloadend = () => {
        ...
        setShowLogo(true);
        try {
          localStorage.setItem("emlaknomi_custom_logo", base64Data);
        } catch (err) {}
      };
  ...
  const generateDescription = () => {
    ...
    if (!generatedTitle) {
      ...
      if (
        flLower.includes("zemin") ||
        flLower.includes("giriş") ||
        flLower.includes("dükkan") ||
        flLower.includes("bodrum")
      ) {
        floorType = fl;
      ...
      generatedTitle = `${formData.neighborhood}'da ${
        formData.rooms || ""
      } ${floorType} ${formData.type}`;
    }
    ...
    Object.keys(featureCategories).forEach((cat) => {
      const selectedInCat = featureCategories[cat].filter((f) =>
    ...
    const desc =
      `EMLAKNOMİ'DEN ${generatedTitle.toUpperCase()}\n\n` +
    ...
    if (canvas) {
      canvas.toBlob((blob) => {
        window.saveAs(
          blob,
          `Emlak_Tasarim_${
            formData.title ? formData.title.substring(0, 10) : "Taslak"
          }.png`
        );
      });
    } else {
  ...
  const handleDownloadProject = async () => {
    if (!window.JSZip || !window.saveAs || !window.html2canvas) {
      alert(
        "İndirme kütüphaneleri henüz yüklenmedi, lütfen sayfayı yenileyin."
      );
      return;
    }
    ...
    const safeTitle =
      formData.title.replace(/[^a-z0-9]/gi, "_").substring(0, 20) ||
      "Emlak_Projesi";
    ...
    metinFolder.file(
      "ilan_metni.txt",
      formData.description || "Metin oluşturulmadı."
    );

    const ozelFolder = rootFolder.folder("4_OZEL_BILGI");
    const ozelContent = `MÜŞTERİ BİLGİ VE TAKİP FORMU
-------------------------------------------
...
`;
  ...
  const availableDistricts = detailedCities.includes(formData.city)
    ? Object.keys(locationData[formData.city] || {})
    : [];
  const availableNeighborhoods = detailedCities.includes(formData.city)
  ...
  const getFeatureValue = (category, featureName) => {
    if (category === "Cephe") {
      return Array.isArray(formData.facade)
        ? formData.facade.join(", ")
        : formData.facade;
    }
    if (category === "Asansör") {
    ...
    if (featureName) {
      return formData.features.includes(featureName) ? "Var" : "-";
    }
    return "-";
  };
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-xl font-bold">
              Özcan AKTAŞ - Emlaknomi{" "}
              <span className="text-orange-500 font-light">Pro</span>
            </h1>
          </div>
          <div className="flex items-center space-x-4">
            <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center font-bold text-sm transition-colors disabled:opacity-50"
            >
              {isDownloading ? (
                "Hazırlanıyor..."
              ) : (
                <>
                  <Download size={18} className="mr-2" /> Projeyi İndir (ZIP)
                </>
              )}
            </button>
            <div className="text-right hidden md:block">
              ...
              <div className="text-[10px] text-slate-500 mt-0.5">
                TTYB No: {currentOffice.authNo}
              </div>
            </div>
            <div className="h-8 w-8 bg-orange-500 rounded flex items-center justify-center font-bold border border-orange-600">
              ÖA
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 print:hidden">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h2 className="text-lg font-semibold mb-4 flex items-center text-slate-700">
              <User className="mr-2 text-blue-600" size={20} /> Ofis ve Marka
              Ayarları
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Çalışılan Şube/Bölge
                </label>
                <select
                ...
                <div className="mt-2 text-xs text-slate-500 bg-slate-50 p-2 rounded">
                  <p>
                    <strong>Tel:</strong> {currentOffice.phone}
                  </p>
                  <p>
                    <strong>Yetki No:</strong> {currentOffice.authNo}
                  </p>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center space-x-2 p-2 border rounded bg-slate-50">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center px-4 py-2 rounded-lg text-xs font-medium transition-all ${
                      showLogo
                  ...
                  >
                    {showLogo ? (
                      ...
                      <>
                        <EyeOff size={14} className="mr-1" /> Göster
                      </>
                    )}
                  </button>
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-2 max-h-40 overflow-y-auto p-1">
                    {formData.images.map((img, idx) => (
                      <div
                        ...
                        onClick={() =>
                          setFormData({ ...formData, coverImageIndex: idx })
                        }
                      >
                        <img
                          src={img}
                        ...
                        {formData.coverImageIndex === idx && (
                          <div
                            className="absolute bottom-0 left-0 w-full text-white text-[9px] text-center font-bold"
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-500 bg-slate-100 p-2 rounded-l border border-r-0 whitespace-nowrap">
                    Emlaknomi'den
                  </span>
                  <input
                    type="text"
                    ...
                    className="w-full p-2 border border-slate-300 rounded-r"
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-bold text-slate-700 flex items-center">
                    <MapPin size={16} className="mr-1 text-orange-500" /> Konum
                    Seçimi
                  </label>
                  {detailedCities.includes(formData.city) && (
                    <button
                      onClick={() => setIsManualLocation(!isManualLocation)}
                      className="text-xs text-blue-600 underline flex items-center hover:text-blue-800"
                    >
                      {isManualLocation
                        ? "Listeye Dön"
                    ...
                    <div>
                      ...
                      >
                        {availableDistricts.length > 0 ? (
                          ...
                          <option disabled>İlçe Yok</option>
                        )}
                      </select>
                    </div>
                    <div>
                      ...
                      {availableNeighborhoods.length > 0 ? (
                        ...
                        <input
                          ...
                          disabled
                          value="Manuel Giriş"
                          className="w-full p-2 border bg-slate-100 rounded-md text-sm"
                        />
                      )}
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  ...
                  >
                    <option>Satılık Daire</option>
                    <option>Kiralık Daire</option>
                    <option>Satılık Ofis</option>
                    <option>Kiralık Ofis</option>
                    <option>Kiralık İşyeri</option>
                    <option>Satılık Arsa</option>
                    <option>Satılık Tarla</option>
                    <option>Satılık Bahçe</option>
                    <option>Satılık Villa</option>
                    <option>Satılık Müstakil Ev</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-slate-500 mb-1 font-bold text-red-500">
                    İlan No
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-slate-500 mb-1 font-bold text-blue-600">
                    Bulunduğu Kat
                  </label>
                  <select
                    name="floor"
                    ...
                    onChange={handleInputChange}
                    className="w-full p-2 border-2 border-blue-100 rounded-lg text-sm"
                  >
                    <option value="">Seçiniz</option>
                ...
                <div>
                  ...
                  <select
                    name="flatCountOnFloor"
                    value={formData.flatCountOnFloor}
                    onChange={handleInputChange}
                    className="w-full p-2 border rounded-lg text-sm"
                  >
                    <option value="">Seçiniz</option>
                    {options.flatCount.map((f) => (
                      <option key={f}>{f}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="relative group">
                  ...
                  <div className="w-full p-2 border rounded-lg text-sm h-10 overflow-hidden cursor-pointer bg-white group-hover:h-auto absolute z-10 shadow-sm">
                    {options.facade.map((f) => (
                      <div
                        key={f}
                        onClick={() => handleMultiSelect("facade", f)}
                        className={`flex items-center p-1 hover:bg-slate-100 ${
                          formData.facade.includes(f) ? "font-bold" : ""
                        }`}
                        style={{
                          color: formData.facade.includes(f)
                            ? themeColor
                            : "inherit",
                        }}
                      >
                        {formData.facade.includes(f) && (
                ...
                <div className="relative group">
                  ...
                  <div className="w-full p-2 border rounded-lg text-sm h-10 overflow-hidden cursor-pointer bg-white group-hover:h-auto absolute z-10 shadow-sm">
                    {options.heating.map((h) => (
                      <div
                        key={h}
                        onClick={() => handleMultiSelect("heating", h)}
                        className={`flex items-center p-1 hover:bg-slate-100 ${
                          formData.heating.includes(h) ? "font-bold" : ""
                        }`}
                        style={{
                          color: formData.heating.includes(h)
                            ? themeColor
                            : "inherit",
                        }}
                      >
                        {formData.heating.includes(h) && (
                          <CheckCircle size={10} className="mr-1" />
                        )}{" "}
                        {h}
                      </div>
                ...
                <div>
                  ...
                  <select
                    name="glassBalcony"
                    value={formData.glassBalcony}
                    onChange={handleInputChange}
                    className="w-full p-2 border rounded-lg text-sm"
                  >
                    <option value="">Seçiniz</option>
                    {options.glassBalcony.map((g) => (
                ...
                <div className="relative group">
                  ...
                  <div className="w-full p-2 border rounded-lg text-sm h-10 overflow-hidden cursor-pointer bg-white group-hover:h-auto absolute z-10 shadow-sm">
                    {options.pantry.map((p) => (
                      <div
                        key={p}
                        onClick={() => handleMultiSelect("pantry", p)}
                        className={`flex items-center p-1 hover:bg-slate-100 ${
                          formData.pantry.includes(p) ? "font-bold" : ""
                        }`}
                        style={{
                          color: formData.pantry.includes(p)
                            ? themeColor
                            : "inherit",
                        }}
                      >
                        {formData.pantry.includes(p) && (
                          <CheckCircle size={10} className="mr-1" />
                        )}{" "}
                        {p}
                      </div>
                        ...
                        <div className="p-3 flex flex-wrap gap-2 max-h-60 overflow-y-auto">
                          {featureCategories[category].map((feature) => (
                            <button
                              ...
                              className={`px-3 py-1.5 rounded text-xs font-medium border transition-all text-left ${
              ...
              <div className="pt-2 border-t border-slate-100">
                <button
                  ...
                  className="w-full py-3 text-white rounded-lg hover:opacity-90 transition-all font-bold flex justify-center items-center shadow-md"
                  style={{
                    background: `linear-gradient(to right, ${themeColor}, #000)`,
                  }}
                >
                  <FileText size={18} className="mr-2" /> Sihirli Metin Oluştur
                </button>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  rows={8}
                  className="w-full p-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:outline-none font-mono text-slate-600 whitespace-pre-line"
                  style={{ focusRingColor: themeColor }}
                  placeholder="Yapay zeka metni buraya gelecek..."
                />
              </div>
              ...
              <div className="mt-6 border-t-2 border-slate-200 pt-4 bg-red-50 p-4 rounded-xl border-dashed border-red-200">
                <div className="flex items-center text-red-600 font-bold mb-2">
                  <Lock size={18} className="mr-2" /> Gizli / Özel Bilgiler
                  (Sadece Klasöre Kaydedilir)
                </div>
                <div className="text-xs text-red-400 mb-4">
                  Bu alana girdiğiniz bilgiler ilan metninde veya görsellerde{" "}
                  <u>ASLA</u> görünmez. Sadece indirilen ZIP dosyasının içindeki
                  özel metin belgesinde yer alır.
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="customerName"
                      value={privateData.customerName}
                      onChange={handlePrivateInputChange}
                      className="w-full p-2 border border-slate-300 rounded-lg text-sm"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="finalPrice"
                      value={privateData.finalPrice}
                      onChange={handlePrivateInputChange}
                      className="w-full p-2 border border-slate-300 rounded-lg text-sm"
                      placeholder="Pazarlıksız son fiyat"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">
                    ...
                    <input
                      type="text"
                      name="commission"
                      value={privateData.commission}
                      onChange={handlePrivateInputChange}
                      className="w-full p-2 border border-slate-300 rounded-lg text-sm"
                      placeholder="%2 veya Net Rakam"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">
                      Gayrimenkul Taşınmaz No
                    </label>
                    <input
                      type="text"
                      name="propertyNo"
                      value={privateData.propertyNo}
                      onChange={handlePrivateInputChange}
                      className="w-full p-2 border border-slate-300 rounded-lg text-sm"
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full p-2 border border-slate-300 rounded-lg text-sm"
                      placeholder="Sadece size özel notlar..."
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 inline-flex w-full print:hidden">
            <button
              ...
              className={`flex-1 py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center ${
                activeTab === "social" ? "text-white" : "text-slate-500"
              }`}
              style={
                activeTab === "social" ? { backgroundColor: themeColor } : {}
              }
            >
              <Instagram size={18} className="mr-2" /> Sosyal Medya
            </button>
            <button
              onClick={() => setActiveTab("whatsapp")}
              className={`flex-1 py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center ${
                activeTab === "whatsapp" ? "text-white" : "text-slate-500"
              }`}
              style={
                activeTab === "whatsapp" ? { backgroundColor: "#25D366" } : {}
              }
            >
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center ${
                activeTab === "print" ? "text-white" : "text-slate-500"
              }`}
              style={
                activeTab === "print" ? { backgroundColor: themeColor } : {}
              }
            >
              <Printer size={18} className="mr-2" /> Vitrin
            </button>
          </div>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-2">
                  <button
                    ...
                    className="bg-blue-100 hover:bg-blue-200 text-blue-800 text-xs px-3 py-1 rounded-full border border-blue-300 transition-colors flex items-center mr-2 font-bold"
                  >
                    <Download size={14} className="mr-1" /> Görseli İndir
                  </button>
                  <button
                  ...
                  >
                    <ImageIcon size={14} className="mr-1" /> Tekli
                  </button>
                  <button
                    onClick={() => setDesignMode("multi")}
                    className={`text-xs px-3 py-1 rounded-full border transition-colors flex items-center ${
                      designMode === "multi"
                        ? "text-white"
                        : "bg-white text-slate-600"
                  ...
                  >
                    <Grid size={14} className="mr-1" /> Çoklu
                  </button>
                </div>
              </div>
              <div
                ref={socialPreviewRef}
                className="aspect-square w-full max-w-md mx-auto bg-slate-900 overflow-hidden relative shadow-2xl"
              >
                {designMode === "single" ? (
                  <img
                    src={
                  ...
                  <div className="grid grid-cols-2 grid-rows-2 w-full h-full">
                    {[0, 1, 2, 3].map((i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="w-full h-full border-[0.5px] border-white/10 relative overflow-hidden"
                        >
                          <img
                            src={img}
                            className="w-full h-full object-cover"
                ...
                <div className="absolute top-6 left-0 flex flex-col items-start gap-1 z-20">
                  <div
                    className="text-white px-4 py-1 text-sm font-bold shadow-md rounded-r-lg tracking-wide"
                    style={{ backgroundColor: themeColor }}
                  >
                    {formData.type.toLocaleUpperCase("tr-TR")}
                  </div>
                  <div
                    className="bg-slate-900/80 text-white px-3 py-1 text-xs font-medium shadow-md rounded-r-lg backdrop-blur-sm border-l-2"
                    style={{ borderColor: themeColor }}
                  >
                    {formData.neighborhood} Mh.
                  </div>
                  {formData.adNumber && (
                ...
                <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black/95 via-black/70 to-transparent text-white z-20">
                  <h2 className="text-2xl font-bold leading-tight mb-2">
                    Emlaknomi'den {formData.title} {formData.rooms}
                  </h2>
                  ...
                  <div className="flex justify-between items-center mb-3 border-t border-white/20 pt-3">
                    <div className="flex space-x-3 text-sm font-medium">
                      <span className="flex items-center">
                        <Home size={12} className="mr-1 opacity-70" />
                        {formData.rooms}
                      </span>
                      <span className="w-px h-3 bg-white/30"></span>
                      <span className="flex items-center">
                        <Layout size={12} className="mr-1 opacity-70" />
                        {formData.size} m²
                      </span>
                      <span className="w-px h-3 bg-white/30"></span>
                      <span className="flex items-center">
                    ...
                    <div className="text-base font-bold bg-white/20 px-2 py-1 rounded whitespace-nowrap text-orange-600">
                      {formData.price} {formData.currency}
                    </div>
                  </div>
                  <div
                    className="text-white text-sm font-bold py-1.5 px-3 rounded w-full text-center whitespace-nowrap shadow-lg flex items-center justify-center"
                    style={{
                      background: `linear-gradient(to right, ${themeColor}, #000)`,
                    }}
                  >
                    <User size={14} className="mr-2" /> Özcan AKTAŞ -{" "}
                    {currentOffice.phone}
                  </div>
                </div>
              </div>
          ...
          {activeTab === "whatsapp" && (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 animate-in fade-in duration-300 print:hidden">
              ...
              <div className="bg-[#e7fce3] p-4 rounded-lg border border-[#d1eecf] text-slate-800 font-mono text-sm whitespace-pre-wrap relative max-h-[500px] overflow-y-auto">
                {formData.description
                  ? formData.description
                  : "Önce sol taraftan 'Sihirli Metin Oluştur' butonuna basınız."}
              </div>
              <button
                onClick={() => copyToClipboard(formData.description)}
                className="mt-4 w-full py-3 bg-[#25D366] text-white rounded-lg hover:bg-[#128C7E] transition-colors flex justify-center items-center shadow-md font-bold"
              >
                <Copy size={18} className="mr-2" /> Mesajı Kopyala
              </button>
            </div>
          )}
          ...
          {activeTab === "print" && (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 animate-in fade-in duration-300 print:hidden">
              ...
              <div className="aspect-[1/1.414] w-full bg-white border border-slate-200 shadow-lg p-4 flex flex-col relative overflow-hidden scale-95 origin-top">
                ...
                >
                  {showLogo ? (
                    <div className="h-10 w-20 flex items-center justify-start">
                      <img
                        src={customLogo || FIXED_LOGO_URL}
                    ...
                    <div className="font-bold text-slate-700">EMLAKNOMİ</div>
                  )}
                  <div className="text-[10px] text-right">
                    <div className="font-bold">Özcan AKTAŞ</div>
                    <div>{currentOffice.phone}</div>
                  </div>
                </div>
                <div className="w-full h-48 bg-slate-100 mb-2 overflow-hidden rounded-sm relative">
                  ...
                  />
                  <div
                    className="absolute top-0 right-0 text-white text-xs px-2 py-1 font-bold"
                    style={{ backgroundColor: themeColor }}
                  >
                    {formData.type}
                  </div>
                </div>
                <h1 className="text-lg font-bold text-slate-800 mb-1 leading-tight">
                  Emlaknomi'den {formData.title} {formData.rooms}
                </h1>
                <div className="text-xs text-slate-500 mb-2">
                  {formData.neighborhood}, {formData.district} / {formData.city}
                </div>
                <div className="grid grid-cols-4 gap-2 text-[9px] mb-2">
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">ODA</span>
                    <strong>{formData.rooms}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">KAT</span>
                    <strong>{formData.floor}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">M²</span>
                    <strong>{formData.size}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">YAŞ</span>
                    <strong>{formData.age}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">
                      CEPHE
                    </span>
                    <strong>
                      {Array.isArray(formData.facade)
                        ? formData.facade.join(", ")
                        : formData.facade}
                    </strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    ...
                    <strong>{formData.elevator}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">
                      TAKAS
                    </span>
                    <strong>{formData.swapAvailable}</strong>
                  </div>
                  <div className="bg-slate-50 p-1 border rounded text-center">
                    <span className="block text-slate-400 text-[8px]">
                      KREDİ
                    </span>
                    <strong>{formData.creditSuitable}</strong>
                  </div>
                </div>
                <div className="mt-auto bg-slate-900 text-white text-center py-2 rounded">
              ...
              <button
                ...
                className="mt-4 w-full py-3 text-white rounded-lg hover:opacity-90 transition-colors flex justify-center items-center shadow-md font-bold"
                style={{ backgroundColor: themeColor }}
              >
                <Printer size={18} className="mr-2" /> Yazdır (A4)
              </button>
            </div>
          )}
        </div>
